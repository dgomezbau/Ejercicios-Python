...
def guardarTablero(path, tablero):
    save = open(path, 'w')
    for i in range(len(tablero)):
        for j in range(len(tablero[i])):
            save.write(tablero[i][j])
    save.close()

def cargarTablero(path, tamano):
    load = open(path, 'r')
    tablero = []
    for x in range(tamano):
        tablero.append([])
        for y in range(tamano):
            tablero[x].append(load.read(2))
    load.close()
    return tablero

def cargarTablero_r(path, tamano):
    load = open(path, 'r')
    tablero = []
    for x in range(tamano):
        tablero.append([])
        for y in range(tamano):
            tablero[x].append(load.read(1))
    load.close()
    return tablero
...
def comprueba_user_input(coordenada, tamano):
    if coordenada[0]>tamano and coordenada[1]>tamano and coordenada[0]!=0 and coordenada[1]!=0:
        return True
    else:
        return False
...
while status(tablero_j) and status(tablero_o):
    ...
    while(acierto_j and status(tablero_o)):
        ...
        opcion = input('¿Quieres guardar o cargar? (y/n): ')
        if opcion == 'y':
            opcion2 = input('guardar (s), cargar(l): ')
        
            if opcion2 == 's':
                guardarTablero('D:\\Curso Python MC\\Ejercicios Python\\Hoja06\\barcos_j.txt', tablero_j)
                guardarTablero('D:\\Curso Python MC\\Ejercicios Python\\Hoja06\\barcos_o.txt', tablero_o)
                guardarTablero('D:\\Curso Python MC\\Ejercicios Python\\Hoja06\\radar_j.txt', radar_j)
                guardarTablero('D:\\Curso Python MC\\Ejercicios Python\\Hoja06\\radar_o.txt', radar_o)
            elif opcion2 == 'l':
                tablero_j = cargarTablero('D:\\Curso Python MC\\Ejercicios Python\\Hoja06\\barcos_j.txt', tamano_tablero)
                tablero_o = cargarTablero('D:\\Curso Python MC\\Ejercicios Python\\Hoja06\\barcos_o.txt', tamano_tablero)
                radar_j = cargarTablero_r('D:\\Curso Python MC\\Ejercicios Python\\Hoja06\\radar_j.txt', tamano_tablero)
                radar_o = cargarTablero_r('D:\\Curso Python MC\\Ejercicios Python\\Hoja06\\radar_o.txt', tamano_tablero)
                
        
        coord_jugador = input_jugador(input('Introduce fila y columna(Ej: 3,2): '))
