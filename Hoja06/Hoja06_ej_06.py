...
def conversionACaracteres(matriz):
    for i in range(len(matriz)):
        for j in range(len(matriz)):
            if matriz[i][j] == '1':
                matriz[i][j] = "■"
...
def guardarTablero(path, tablero):
    tablero_c = conversionAUnos(tablero)
    save = open(path, 'w')
    for i in range(len(tablero_c)):
        for j in range(len(tablero_c[i])):
            save.write(tablero_c[i][j])
    save.close()
    conversionACaracteres(tablero)
...
def cargarTablero(path, tamano):
    ...
    tablero_c = conversionACaracteres(tablero)
    load.close()
    return tablero_c
...
def menu_tx():
    print('--------------------------------------------\n'
          ...
          + '6 - Recupera el último tablero jugado\n'
          + '--------------------------------------------\n'
          + '0 - Salir')
...
while menu !=0:
    ...
    if menu == 1:
        ...
        if tamano_tablero<=20:
            ...
            guardarTablero('D:\\Curso Python MC\\Ejercicios Python\\Hoja06\\JVPartida.txt', tablero_actual)
        ...
        tablero_actual = cargarTablero('D:\\Curso Python MC\\Ejercicios Python\\Hoja06\\JVPartida.txt', tamano_tablero)
        print_matriz(tablero_actual)          
